...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.0, top_p=1.0, tokens=400, freq_pen=0.0, pres_pen=0.0, stop=['USER:', 'BEN FRANKLIN:']):
    max_retry = 5
    retry = 0
...
if __name__ == '__main__':
    ...
    while True:
        ...
        memories = fetch_memories(vector, conversation, 3)  # pull episodic memories
        ...
        timestring = timestamp_to_datetime(timestamp)
        message = '%s: %s - %s' % ('BEN FRANKLIN', timestring, output)
        info = {'speaker': 'BEN FRANKLIN', 'time': timestamp, 'vector': vector, 'message': message, 'uuid': str(uuid4()), 'timestring': timestring}
        filename = 'log_%s_BEN FRANKLIN.json' % time()
        save_json('chat_logs/%s' % filename, info)
        #### print output
        print('\n\nBEN FRANKLIN: %s' % output) 